...
*/ function burnOwNifty(address _scAddr) public { require(msg.sender == ownerOf(uint(_scAddr))); 
Ownable _sc = Ownable(_scAddr); _sc.transferOwnership(msg.sender); _burn(uint(_scAddr)); } /** 
* @todo Create function that can perform delegate call to contract * with tokenized ownership on owner behalf */
